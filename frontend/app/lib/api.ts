...
// ── Courses API ───────────────────────────────────────────────────────────────
import type { CourseCreate, CourseResponse, EvaluateEmployeeRequest,
              PerformanceEvaluationResponse, CourseAssignmentResponse, AutoAssignResponse } from './types';

export const createCourse = (course: CourseCreate) =>
    fetchApi<CourseResponse>('/courses/', { method: 'POST', body: JSON.stringify(course) });

export const listCourses = () =>
    fetchApi<CourseResponse[]>('/courses/');

export const deleteCourse = (courseId: string) =>
    fetchApi<void>(`/courses/${courseId}`, { method: 'DELETE' });

// ── Performance API ───────────────────────────────────────────────────────────
export const evaluateEmployee = (employeeId: number, data: EvaluateEmployeeRequest) =>
    fetchApi<PerformanceEvaluationResponse>(`/performance/${employeeId}/evaluate`, {
        method: 'POST', body: JSON.stringify(data) });

export const getEvaluationHistory = (employeeId: number) =>
    fetchApi<any[]>(`/performance/${employeeId}/history`);

export const autoAssignCourses = (employeeId: number, projectId: number) =>
    fetchApi<AutoAssignResponse>(`/performance/${employeeId}/auto-assign?project_id=${projectId}`, {
        method: 'POST' });

export const assignCourses = (employeeId: number, courseIds: string[], experienceLevel: string) =>
    fetchApi<CourseAssignmentResponse[]>(`/performance/${employeeId}/assign-courses`, {
        method: 'POST',
        body: JSON.stringify({ course_ids: courseIds, experience_level: experienceLevel }) });

export const getAllProjects = () =>
    fetchApi<import('./types').ProjectResponse[]>('/projects/');
